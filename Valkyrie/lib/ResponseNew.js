...
var send = require('send'); //todo useless?
...
class Response {
  ...
  send(body) {
    ...
    // populate ETag
    let etag;
    const generateETag = typeof len !== 'undefined' && app.get('etag fn');
    if (typeof generateETag === 'function' && !this.get('ETag')) {
      if ((etag = generateETag(chunk))) {
        this.set('ETag', etag);
      }
    }
    ...
    if (app.settings.useContext) this.context.succeed(response);
  ...
  sendStatus(statusCode) {
    ...
    this.type('txt');
    return this.send(statuses[statusCode] || String(statusCode));
  }
   ...
   */
  contentType(type) {
    return this.set('Content-Type', type.includes('/') ? type : mime.lookup(type));
  }
